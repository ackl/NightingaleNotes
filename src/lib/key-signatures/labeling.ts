 ...
 */
const majorKeyLabels: NoteLabel[] = noteLabels
  .flatMap((prev, i) => (i !== 0 && i !== 3
    ? [(prev + accidentals.FLAT) as NoteLabel, prev] : [prev]));
...
const minorKeyLabels = noteLabels.flatMap((prev, i) => {
  if (i === 2 || i === 6) {
    return [(prev + accidentals.FLAT) as NoteLabel, prev];
  }
  if (i === 1 || i === 5) return [prev];

  return [prev, (prev + accidentals.SHARP) as NoteLabel];
});
